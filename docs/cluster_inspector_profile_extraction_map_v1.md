...
| `mpi_providers[*].compatibility.compilers` | module verification | `mpicc -show`, `mpicc -compile-info`, changed compiler drivers, module compiler suffixes, module prereqs, and env vars that establish an exact relationship to an observed compiler provider. A module suite release may map to a compiler whose verified product version differs. Do not copy platform flavor baselines into this field. | exact compiler provider refs such as `intel@2021.13.1` | `probed` / `inferred` | reject the candidate from `mpi_providers` and retain failed-pairing evidence when no exact relationship can be established |
| `mpi_providers[*].flavors.<compiler>.prefix` | platform adapter / module verification | Prefix for a per-compiler MPI flavor when the MPI exposes multiple ABI/layout flavors. The key may be a platform-advertised compiler baseline, such as `gcc@12.3`, and need not identify an installed compiler provider exactly. | absolute prefix | `probed` | omit unavailable flavor; do not duplicate the baseline key into `compatibility.compilers` |
...
| `mpi[*].compiler` | module verification | Decode wrapper output and changed compiler drivers; otherwise map a compiler suffix to one observed provider using its product version, module-suite release, and prefix evidence. Exact versioned module, prefix, or driver-path evidence takes precedence over a wrapper that names only a compiler family. A family-only wrapper result is sufficient only when exactly one observed provider has that family. | compiler spec such as `aocc@4.2.0` | `inferred` from verified relationship evidence | reject the candidate as evidence-only when unknown or ambiguous; never emit an unpaired provider |
...
- A verified MPI identity without one exact compiler pairing remains failed
  candidate evidence and is not promoted into `mpi_providers`.
...
| MPI compiler pairing | `mpi_providers[*].compiler`, `compatibility.compilers`, or `flavors` | Generic MPI discovery admits only exact compiler-provider pairings. Platform MPI flavor keys may instead describe family/version baselines whose exact lane compatibility is resolved by platform policy. Error if a hand-written or malformed provider supplies neither form. |

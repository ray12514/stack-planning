...
An initialized CSE trial workspace also carries a review-only snapshot of the
static catalog configuration it selected, including the exact reviewed
`catalog/profile.yaml` fact sheet. The snapshot is regenerable, but it travels
with the workspace so a second builder does not need the original catalog path
or any of the tool repositories.

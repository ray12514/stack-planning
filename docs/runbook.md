...
| Module exposure | Front-door lane selector modulefiles are rendered. Package modulefiles are still generated by Spack in each lane. | Use the rendered selectors after running Spack module generation, or use temporary shell/view exposure. Record required prereqs and MODULEPATH behavior for the module design. |
...
| front-door/lane selector modules | Generated by `stack-composer render` under the workspace `modulefiles/` tree. | Use after package modules are generated by Spack; inspect prereqs before publishing. |
...
Front-door lane selector modulefiles are generated under
`rendered/<system>/<stack>/<release>/modulefiles/`. They prepend each lane's
package-module root and declare platform module prerequisites. Package
modulefiles are still generated by Spack after install.
...
paths needed by real applications; they are evidence for selector behavior,
